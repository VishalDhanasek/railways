...
import { addWagonAttachment, removeWagonAttachment } from '@/services/alterationService';
import type { AlterationStatus, NewWagonAlterationRecord, WagonAlterationRecord } from '@/types';

const STATUS_OPTIONS: AlterationStatus[] = ['Pending', 'In Progress', 'Completed', 'On Hold'];

interface WagonFormProps {
  open: boolean;
  record: WagonAlterationRecord | null;
  onClose: () => void;
  onSubmit: (data: NewWagonAlterationRecord) => Promise<void>;
  onAttachmentsChanged: () => void;
}

const EMPTY = { date: '', tlNo: '', description: '', status: 'Pending' as AlterationStatus, remarks: '' };

export default function WagonForm({ open, record, onClose, onSubmit, onAttachmentsChanged }: WagonFormProps) {
  ...
  return (
    <Drawer
      open={open}
      onClose={onClose}
      title={record ? 'Edit Wagon Alteration' : 'Add Wagon Alteration'}
    ...
    >
      <div className="space-y-4">
        ...
        <FormField label="TL. No." required error={errors.tlNo}>
          <Input placeholder="e.g. TL/WG/2026/0101" value={form.tlNo} onChange={(e) => set('tlNo')(e.target.value)} invalid={!!errors.tlNo} />
        </FormField>
        ...
        {record && (
          <FormField label="Supporting Documents" hint="Attachments are saved immediately">
            <AttachmentUploader
              ...
              onUpload={async (file) => {
                const updated = await addWagonAttachment(record.id, file);
                setAttachments(updated.attachments);
                onAttachmentsChanged();
              }}
              onRemove={async (attachmentId) => {
                const updated = await removeWagonAttachment(record.id, attachmentId);
                setAttachments(updated.attachments);
                onAttachmentsChanged();
              }}
            />
